...
import { Sparkles, Check, CreditCard, ShieldCheck, Zap, KeyRound, X, Star } from 'lucide-react';
      ...
      <div className="bg-white rounded-3xl p-5 md:p-6 max-w-2xl w-full border border-forest-800/20 shadow-2xl space-y-4 relative overflow-y-auto max-h-[calc(100vh-2rem)] my-auto">
        ...
        <div className="flex items-center justify-between border-b border-forest-800/10 pb-4">
          <div>
            ...
            <h2 className="text-2xl font-serif font-bold text-forest-900 leading-tight">
              Ata Takvimi Premium Abonelik
            </h2>
          </div>
          <button
            onClick={() => setShowSubModal(false)}
            className="p-1.5 text-forest-800/40 hover:text-forest-900 font-bold text-lg rounded-xl hover:bg-forest-800/5 transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs bg-forest-50/70 p-4 rounded-2xl border border-forest-800/10">
          <div className="flex items-start gap-2">
            <div className="w-5 h-5 rounded-full bg-harvest-400 text-forest-900 flex items-center justify-center shrink-0 mt-0.5 font-bold text-[10px]">✓</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          >
            ...
            <div className="mt-4 pt-2 border-t border-forest-800/10 flex items-center justify-between text-xs font-bold text-forest-900">
          ...
          >
            ...
            <div>
              ...
              <p className="text-[11px] text-white/80">Ayda sadece <strong>₺83</strong>! Tüm sezon boyunca 81 il hava uyarısı ve don takibi.</p>
            </div>
        ...
        {/* Action Button: İyzico Payment / Direct Activation */}
        <button
          onClick={handleActivate}
          className="w-full py-4 rounded-2xl badge-forest font-bold text-sm shadow-xl hover:scale-[1.01] transition-transform flex items-center justify-center gap-2 group"
        >
          <CreditCard className="w-5 h-5 text-harvest-400 group-hover:rotate-12 transition-transform" />
          <span>İyzico Güvenli Ödeme İle Aboneliği Başlat (₺{selectedPlan === 'annual' ? '999' : '149'})</span>
        </button>

        {/* License Code Entry */}
        <div className="border-t border-forest-800/10 pt-4 space-y-2">
          ...
          <form onSubmit={handlePromoSubmit} className="flex gap-2 text-xs">
            <input
              ...
              className="flex-1 bg-forest-50 border border-forest-800/20 rounded-xl p-3 font-bold text-forest-900 outline-none focus:border-harvest-500 uppercase"
            />
            <button
              type="submit"
              className="px-4 py-3 bg-forest-800 text-white font-bold rounded-xl hover:bg-forest-900 transition-colors shrink-0"
            >
              Kodu Kullan
            </button>
          </form>
        ...
        {/* Guarantee Note */}
        <div className="flex items-center justify-center gap-2 text-[11px] text-forest-800/60 pt-1">
